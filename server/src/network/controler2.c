...
int verify_and_load_player(int player_id, int fd, player** p){
	...
	if(((*p) = find_player_by_fd(p_list, fd))) {
		printf("Player with fd %d already exists\n", fd);
		if(verify_player(*p, player_id))
			return 0;
		else
			return 1;
	} else {
		if(player_id == 0) //id = 0 means players might be trying to CONNECT
			return 0;
		else
			return 1;
	}
...
char* create_lobby(player* p, char* lobby_name) {
	...
	return construct_message(p->id, 201,
...
/*
	char *response = construct_message(0, 201, "You were disconnected");


	int rv = send(p->socket, response, strlen(response), MSG_CONFIRM);
